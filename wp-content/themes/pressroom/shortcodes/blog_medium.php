<?php
//blog
function pr_theme_blog_medium($atts, $content)
{
	global $themename;
		...
		while (have_posts()) : the_post();
			...
			if($i==0 || ($featured_post!="-" && $i==1))
			{
				if($featured_post!="-" && $i==1)
					$output .= '</ul>';
				$output .= '<ul class="blog clearfix' . ($featured_post!="-" && $i==1 ? (substr($post_list_style,0,6)=="simple" ? " list" : " small") : " small_margin") . ($top_margin!="none" ? ' ' . $top_margin : '') . ($el_class!="" ? ' ' . $el_class : '') . '">';
			}
