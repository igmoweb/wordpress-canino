<?php
//blog
function pr_theme_blog_2_columns($atts, $content)
{
	global $themename;
	...
	$args = array( 
		...
		'post_type' => 'post',
		'post_status' => 'publish',
		'paged' => ((int)$pr_pagination ? $paged : 0),
		...
		while (have_posts()) : the_post();
			...
			/*if($i==0 || (($i==ceil($post_count/2) && $featured_post=="-") || ($featured_post!="-" && $i==1)))
			{
				if(($i==ceil($post_count/2) && $featured_post=="-") || ($featured_post!="-" && $i==1))
					$output .= '</ul></div>';*/
				$output .= '<div class="vc_col-sm-6 wpb_column vc_column_container"><ul class="blog clearfix' . ($featured_post!="-" && $i==1 ? " small" : "") . ($top_margin!="none" ? ' ' . $top_margin : '') . ($el_class!="" ? ' ' . $el_class : '') . '">';
			/*}*/
